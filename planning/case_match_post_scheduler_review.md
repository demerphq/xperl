...
In `case ($x)`, the subject lexical is implicitly pinned throughout the
data-shape. A direct `match($x)` is therefore an identity match, and a nested
use such as `match([$x])` is not a capture of the array element. It asks for
an array containing the complete case subject and, in the ordinary case,
can never match.

The compiler should reject such a clause with a diagnostic along the lines
of:

    The case subject $x is implicitly pinned; this match clause can never match

The guide and focused tests should cover direct identity, nested use, subject
aliases, and coerced or computed subjects. A later diagnostic improvement
should include the source text of the offending match clause; that is useful
context but is not required for the initial implementation.
