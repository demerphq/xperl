...
- C source files may have an opening poem or quotation, such as the Lord of
  the Rings quotation in `run.c`. When modifying one, keep that quotation in
  its established position. Only `#include` directives should precede it;
  declarations, functions, and other code should remain below it.
