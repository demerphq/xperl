...
For a step-by-step introduction, see L<perlcasematchtut>. The full reference
is L<perlcasematch>.
...
order. The first match clause that matches runs; there is no fall-through. The wildcard
data shape C<_> matches any value and is useful as a final match clause:
...
The subject may be explicitly coerced before matching with C<ToInteger>,
C<ToFloat>, or C<ToString>:
...
subject once, before any match clause is tested. The type word is part of the case
...
Numeric criteria can distinguish native numbers from numeric-looking strings:

    case ($value) {
        match (IntStr($integer)) { ... }
        match (FloatStr($real))  { ... }
        match (Num($number))     { ... }
        match (NumStr($numeric)) { ... }
    }

C<IntStr()> and C<FloatStr()> accept their corresponding native values and
numeric strings, including surrounding whitespace and leading-zero padding.
C<Num()> accepts only native integer or floating-point values.  C<NumStr()> is
the union of the two string-bearing forms.  A target receives the original
value; matching C<"0007"> does not turn the binding into C<7>.

C<Strict(...)> can wrap C<IntStr()>, C<FloatStr()>, or C<NumStr()> to reject
surrounding whitespace and non-canonical leading-zero padding.  Applying
C<Strict()> to C<Num()> is a compile-time error.  With C<NumEq(...)>,
C<Strict()> instead requires that a string begin with a numeric prefix
before comparing it numerically; see L<perlcasematch> for examples.

C<NumEq(EXPR)> is a matching-only criterion.  For now, C<EXPR> must be a
literal value.  It compares using Perl's normal numeric C<==> rules and does
not create a destructuring binding.  Consequently,
C<NumEq(0)> matches strings such as C<"0000"> and C<"0.0">, while a value such
as C<"A"> is treated as zero with Perl's usual numeric warning.

Zero-argument function and method calls may also supply a scalar data-shape
value. They are called in scalar context when their match clause is tried:
...
This is a limited exception to the rule that the left side of C<match> is a
data-shape description rather than ordinary Perl code.  Calls with arguments
are rejected; use a guard after C<if> for ordinary Perl computation.

A backslash can describe a scalar reference shape.  For example,
C<match(\$value)> matches a reference to a scalar and binds the referred-to
value; C<match(\\$value)> describes one additional reference layer.  This is
different from C<RefVal($value)>, which binds the outer reference itself.

String data shapes may contain literal fragments and multiple unpinned scalar
...
captures.  Multiple captures are supported when literal or pinned fragments
separate them.  The concatenation is data-shape syntax; it is not evaluated as
an ordinary concatenation before matching.  A name pinned by C<with>
contributes its existing value instead of capturing.  General expression
operands are not currently supported.

A regular expression can also be used as a data shape.  Its ordinary capture
variables remain available, and a named capture such as C<(?<name>...)> also
creates the match-clause-local scalar C<$name>. A named capture that does not
participate in the successful match is bound to C<undef>.
...
Blessed references can use a class-qualified shape.  For example,
C<match(Point { '$x' => $x, '$y' => $y })> checks a blessed hash object and
binds its named fields, while C<match(Pair [ $first, $second ])> checks a
blessed array object.  C<match(Box \$value)> checks a blessed scalar reference
and binds its referent.  These checks inspect the reference structure directly;
they do not call constructors, accessors, overload methods, or arbitrary user
methods.  A hash object shape is exact unless it ends with C<...>.
...
An existing scalar can also be pinned at the point where it appears in a
data shape by prefixing it with C<^>:

    my $wanted_key = "key";
    case ($record) {
        match ({ ^$wanted_key => $value, ... }) { use_record($value) }
    }

In a C<match (...)> data shape, C<^$name> compares with the existing lexical's
value instead of introducing a match-clause-local binding. The value is captured
when the surrounding C<case> begins.  This is a pattern-only meaning of the
caret; ordinary Perl expressions, including guards, retain the normal caret
operators.
