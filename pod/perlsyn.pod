...
value.  They are called in scalar context when their clause is tried:
...
creates the clause-local scalar C<$name>.  A named capture that does not
...
value instead of introducing a clause-local binding.  The value is captured
