...
L<C<__NAMESPACE__>|/__NAMESPACE__>, L<C<next>|/next LABEL>,
L<C<__PACKAGE__>|/__PACKAGE__>,
...
the yielded list in list context or its first value in scalar context.  The
...
The generator keywords can be enabled with C<use feature 'generator'>.  For
the complete generator interface, including the predicate builtins, use the
combined C<generator> pragma:

The body can use C<wantarray> to observe that context.  A list, scalar, or
void call gives the resumed body list, scalar, or void context respectively;
the context may change between calls.
...
Generators can be interleaved explicitly, with each generator retaining its
own continuation:
...
empty list and the generator becomes exhausted.  Later calls also return an
empty list; normal exhaustion is permanent and harmless.  An empty yield is
distinct from exhaustion and can be used to suspend without producing values.

A generator accepts arguments on its first call as ordinary subroutine
arguments.  Arguments passed to a later call are returned by the suspended
C<generator_yield>, with the first argument returned in scalar context and all
arguments returned in list context.  It is a one-shot continuation: it cannot
be resumed while it is already running, after exhaustion, or after an uncaught
exception.  An exception caught by an C<eval> inside the generator does not
exhaust it; an uncaught exception is propagated by the call that resumes the
generator and permanently puts it into the failed state.

This feature is experimental and may change or be removed in a future Perl
release.

=item generator_yield LIST
X<generator_yield>

=for Pod::Functions suspend a generator and produce values

The experimental C<generator> feature provides C<generator_yield> as a keyword
that suspends the enclosing C<generator_create> block until the generator is
called again.  LIST is evaluated in list context, so a yield can produce zero,
one, or many values.  An empty yield is a real suspension and is distinct from
exhaustion.  In scalar context the first yielded value is returned; in list
context all yielded values are returned.  C<undef> is a valid yielded value.

    my $g = generator_create {
        my $response = generator_yield "ready";
        return $response;
    };

    my $state = $g->();          # "ready"
    my $result = $g->("done");  # "done"

Arguments passed to a later generator call are returned by the suspended
...
discarded.  C<generator_yield> is only valid lexically inside a
C<generator_create> block.  It cannot be used in an ordinary subroutine or in
callback code such as C<sort>, C<map>, C<grep>, or a regular-expression code
block.

This keyword is experimental and requires C<use feature 'generator'>.
...
Returns true if EXPR is a generator that can no longer run.  It returns false
for a new or suspended generator, and does not resume or advance the
generator.  A failed generator is also exhausted, even though it did not
complete normally.  This experimental builtin can be imported with C<use
builtin> or C<use generator>.

=item generator_completed EXPR
X<generator_completed>

=for Pod::Functions test whether a generator completed normally

Returns true if EXPR is a generator that reached the end of its block or
returned normally.  It returns false for a new or suspended generator and for
a generator that failed with an uncaught exception.  This experimental builtin
can be imported with C<use builtin> or C<use generator>.

=item generator_failed EXPR
X<generator_failed>

=for Pod::Functions test whether a generator failed

Returns true if EXPR is a generator that stopped because of an uncaught
exception.  It does not resume the generator or consume the exception.  This
experimental builtin can be imported with C<use builtin> or C<use generator>.

=item generator_running LIST
X<generator_running>

=for Pod::Functions return generators that can still run

Returns the original generator references from LIST that are not exhausted.
A generator that yielded an empty list is still returned because it can
continue.  Normally completed and failed generators are omitted.  This is
useful for advancing several generators in turns:

    while (@generators = generator_running(@generators)) {
        $_->() for @generators;
    }

This experimental builtin can be imported with C<use builtin> or C<use
generator>.
...
=for Pod::Functions the current lexical namespace

A special token provided by the experimental C<namespaces> feature.  It
returns the current lexical namespace, which may differ from the package
reported by C<__PACKAGE__>.  See L<perlnamespace>.
