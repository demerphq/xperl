...
block.  Each call resumes the block until its next C<yield>, returning
...
The generator keywords can be enabled with C<use feature 'generator'>.  The
complete object interface is documented in L<perlgenerator>.  The combined
C<generator> pragma enables both C<generator> and C<signatures>, so a generator
can use either ordinary C<@_> arguments or a signature:
...
C<yield>, with the first argument returned in scalar context and all
...
With the combined pragma, a generator can use a signature for its initial
arguments:
    ...
    my $greeting = $g->("Ada");          # "Hello, Ada"
    my $answer   = $g->("Good morning"); # "You said: Good morning"

An ordinary C<@_> body is also supported.  Initial call arguments populate
C<@_>; arguments supplied when resuming are returned by C<yield> instead:
...
The experimental C<generator> feature provides C<yield> as a keyword
that suspends the enclosing C<gen> block until the generator is
...
C<yield>.  In list context all arguments are returned; in scalar
context the first argument is returned, and in void context they are
discarded.  C<yield> is only valid lexically inside a
C<gen> block.  It cannot be used in an ordinary subroutine or in
