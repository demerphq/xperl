...
=head2 role
    ...
    role NAME VERSION BLOCK

    role NAME VERSION : ATTRIBUTES... BLOCK

The C<role> keyword declares a reusable collection of fields, methods, and
C<ADJUST> blocks.  A role cannot be instantiated and therefore does not have a
constructor.  A class or another role consumes it with the C<:implements>
attribute.
...
Roles use the same block and statement forms as classes.  They may consume
other roles, but cannot use C<:isa>.
...
Within a role, a method declaration without a body declares a requirement:

    role Printable {
        method as_string;
        method print { say $self->as_string }
    }

A class consuming this role must provide C<as_string>, either directly or by
inheritance.  Another role may leave the requirement for its eventual
consumer to provide.
...
A class or role composes one or more roles with C<:implements>:

    class Duck :implements(Flyable, Swimmable) { ... }

The attribute may be repeated.  If a named role has not been loaded, Perl
attempts to load it in the same way as for C<:isa>.  A minimum version may be
specified after the role name.

Fields, methods, field initializers, and C<ADJUST> blocks supplied by each role
become part of the consumer.  Roles are composed in declaration order.  A
shared role reached by two paths is composed only once.

Two unrelated roles which provide a method of the same name conflict.  The
consumer must resolve the conflict by declaring that method explicitly.  An
inherited method or an accessor generated by a field attribute does not
resolve it.  Fields of the same name from unrelated roles always conflict.

The C<implements> method performs a nominal membership test:

    $object->implements('Flyable');
    Duck->implements('Flyable');

The same test can be written with the infix operator when the role name is a
bareword:

    if ($object implements Flyable) { ... }

Membership includes roles consumed transitively and roles consumed by a
superclass.  It differs from C<isa>: composing a role does not place that role
in C<@ISA>.  The default C<DOES> implementation reports composed roles as well
as ordinary inheritance relationships.
