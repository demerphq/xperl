...
Roles can be declared with the experimental C<role> keyword and composed into
a class or another role with the C<:implements()> attribute.  A class or role
can be queried with its C<implements()> method, or with the infix
C<implements> operator.  The C<DOES()> method delegates to this query for
class-system objects, while retaining its usual overridable behaviour for
ordinary Perl packages.
