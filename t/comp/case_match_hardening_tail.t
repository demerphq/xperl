...
fresh_perl_is(q{
    ...
    case ($point) {
        match (Point { value => 'target' }) { say 'stringified field' }
    }
}, 'stringified field', {},
...
fresh_perl_is(q{
    ...
    case ($point) {
        match (Point { child => RefVal($captured) }) {
...
fresh_perl_is(q{
    ...
    case ($point) {
        match (Point { value => $value } if $value == 4) {
            die 'failed guard selected';
        }
        match (Point { value => $fallback }) {
