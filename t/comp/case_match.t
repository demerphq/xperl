...
my $implicit_identity;
my $implicit_identity_ok = eval q{
    use feature 'case_match';
...
print !$@ && $implicit_identity_ok && $implicit_identity
    ? "ok 135 - direct subject identity remains valid\n"
    : "not ok 135 - direct subject identity remains valid\n";

my $implicit_subject_array_error = eval q{
    use feature 'case_match';
    my $identity_subject = 42;
    case ($identity_subject) {
        match ([$identity_subject]) { 1 }
    }
    1;
};
print $@ =~ /The case subject \$identity_subject is implicitly pinned; this match clause can never match/
    ? "ok 136 - nested subject capture is rejected\n"
    : "not ok 136 - nested subject capture is rejected\n";

my $implicit_subject_hash_error = eval q{
    use feature 'case_match';
    my $identity_subject = 42;
    case ($identity_subject) {
        match ({ value => $identity_subject, ... }) { 1 }
    }
    1;
};
print $@ =~ /The case subject \$identity_subject is implicitly pinned; this match clause can never match/
    ? "ok 137 - subject capture in a hash shape is rejected\n"
    : "not ok 137 - subject capture in a hash shape is rejected\n";
